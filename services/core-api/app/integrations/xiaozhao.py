...
def import_xiaozhao_payload(session: Session, payload: dict, source_name: str = "xiaozhao-radar") -> ImportSummary:
    ...
    for record in records:
        ...
        if job is None:
            ...
            session.add(job); session.flush(); summary.jobs_created += 1
        else:
            if not job.apply_url and url:
                job.apply_url = url; job.canonical_url = canonical_url; job.status = "DISCOVERED_URL_UNVERIFIED"
            if not job.industry and industry: job.industry = industry
